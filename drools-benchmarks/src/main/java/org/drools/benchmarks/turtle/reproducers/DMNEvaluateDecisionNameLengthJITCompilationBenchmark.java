...
@Fork(3)
public class DMNEvaluateDecisionNameLengthJITCompilationBenchmark extends AbstractBenchmark {

    private static final int WORKS = 10_000;

    @Param({"abigone.dmn", "abigone_longnames.dmn"})
    private String dmnFileName;

    @Param({"false", "true"})
    private String execModelProperty;
    ...
    public void setupResource() throws IOException {
        System.setProperty("org.kie.dmn.compiler.execmodel", execModelProperty);
    ...
    /**
     * Utility main() for profiling purposes
     */
    @SuppressWarnings("resource")
    public static void main(String[] args) throws Throwable {
        DMNEvaluateDecisionNameLengthJITCompilationBenchmark b = new DMNEvaluateDecisionNameLengthJITCompilationBenchmark();
        b.dmnFileName = "abigone_longnames.dmn";
        b.execModelProperty = "false";
        b.setupResource();
        b.setup();
        System.out.println("Ready? Press ENTER:");
        new Scanner(System.in).nextLine();
        DMNResult evaluateDecision = null;
        for (int i = 0; i < WORKS; i++) {
            evaluateDecision = b.evaluateDecision();
        }
        System.out.println(evaluateDecision);
    }
}
