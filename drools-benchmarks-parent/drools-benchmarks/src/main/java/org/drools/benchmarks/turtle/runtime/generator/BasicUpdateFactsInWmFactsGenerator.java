...
public class BasicUpdateFactsInWmFactsGenerator extends FactsGenerator {
    ...
    public BasicUpdateFactsInWmFactsGenerator(final GeneratorConfiguration config) {
        super(config);
    }
    ...
    protected List<Object> generateMatchingFacts(final int totalFacts) {
        ...
        for (int j = 1; j <= outerLoops; j++) {
            for (int i = 1; i <= innerLoops; i++) {
                ...
                addr.setUuid("updateAddress_" + j);
                facts.add(addr);

                final Customer cust = new Customer();
                cust.setUuid("updateCustomer_" + j);
                facts.add(cust);

                final Transaction trans = new Transaction();
                trans.setUuid("updateTransaction_" + j);
                facts.add(trans);

                final Account acc = new Account();
                acc.setUuid("updateAccount_" + j);
                facts.add(acc);

                final CreditCard card = new CreditCard();
                card.setUuid("updateCreditCard_" + j);
                facts.add(card);
            }
        }
        return facts;
